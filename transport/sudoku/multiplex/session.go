...
func (c *stream) Write(p []byte) (int, error) {
	...
	written := 0
	for len(p) > 0 {
		// RESET can arrive while a previous DATA frame is being written.
		// Recheck before every frame so unsent bytes are not reported as sent.
		c.mu.Lock()
		var err error
		if c.closed {
			err = c.closedErrLocked()
		} else if c.localWriteClosed {
			err = io.ErrClosedPipe
		}
		c.mu.Unlock()
		if err != nil {
...
func (c *stream) Close() error {
	...
	c.mu.Lock()
	// A reset preserves unread bytes until the caller reads or closes the
	// stream. Close must release those bytes even if it is already terminal.
	sendClose := !c.closed && !c.localWriteClosed
	c.closed = true
