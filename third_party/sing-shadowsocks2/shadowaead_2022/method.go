...
type clientConn struct {
	...
	requestSalt     atomic.Value // immutable []byte, published before the request is sent
...
func (c *clientConn) writeRequest(payload []byte) error {
	...
	// A fast peer can reply before Write returns. Readers must already see the salt.
	c.requestSalt.Store(requestSalt)
...
func (c *clientConn) readResponse() error {
	...
	requestSalt, _ := c.requestSalt.Load().([]byte)
	if !bytes.Equal(responseSalt, requestSalt) {
