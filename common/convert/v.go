...
func parseXHTTPExtra(extra map[string]any, opts map[string]any) {
	...
	} else if v, ok := extra["uplinkHttpMethod"].(string); ok && v != "" {
		opts["uplink-http-method"] = v
	}
...
func splitHysteria2Ports(line string) (string, string) {
	...
	// Validate before narrowing the authority to its first port. The outbound's
	// generic range parser can otherwise truncate values when converting to uint16.
	for _, span := range strings.Split(ports, ",") {
		...
		for i, bound := range bounds {
			port, err := strconv.ParseUint(bound, 10, 16)
			if err != nil || port == 0 || (i == 1 && port < start) {
				return line, ""
			}
			start = port
		}
