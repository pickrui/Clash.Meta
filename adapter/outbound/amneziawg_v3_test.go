...
func newAmneziaV3PacketDevice(t *testing.T) *amnezia.Device {
	t.Helper()
	w, err := NewWireGuard(WireGuardOption{
		...
		AmneziaWGOption:     &AmneziaWGOption{Version: 3, S1: 8, S2: 16, S3: 24, S4: 64, H1: "101", H2: "102", H3: "103", H4: "104"},
	})
	if err != nil {
		t.Fatal(err)
	}
...
func TestAmneziaV3S4RejectsShortPackets(t *testing.T) {
	device := newAmneziaV3PacketDevice(t)
	for _, size := range []int{0, 1, 3, 4, 31, 32, 63, 64, 65, 66, 67} {
		for _, clipped := range []bool{true, false} {
			t.Run(fmt.Sprintf("size=%d/clipped=%t", size, clipped), func(t *testing.T) {
				...
				if frameSize, kind, padding := device.DeterminePacketTypeAndPadding(packet, make([]byte, 4)); kind != amnezia.MessageUnknownType || frameSize != 0 || padding != 0 {
					t.Fatalf("short packet accepted: type=%d size=%d padding=%d", kind, frameSize, padding)
				}
				if len(packet) != size {
...
func TestAmneziaV3ParsesPaddedPackets(t *testing.T) {
	device := newAmneziaV3PacketDevice(t)
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			frameSize, kind, padding := device.DeterminePacketTypeAndPadding(packet, make([]byte, 4))
			if int(padding) != tc.padding {
				t.Fatalf("padding=%d want=%d", padding, tc.padding)
			}
			packet = packet[padding:]
			if kind != amnezia.MessageTransportType {
				packet = packet[:frameSize]
			}
			if kind != tc.kind-100 || !bytes.Equal(packet, expected) {
