...
import (
	...
	snellListener "github.com/metacubex/mihomo/listener/snell"
	"github.com/metacubex/mihomo/log"
	snellTransport "github.com/metacubex/mihomo/transport/snell"
)
...
func NewSnell(options *SnellOption) (*Snell, error) {
	...
	if options.Version == 0 {
		options.Version = snellTransport.Version4
	}
	if options.Version != snellTransport.Version4 && options.Version != snellTransport.Version5 {
		return nil, fmt.Errorf("snell inbound version %d is not supported", options.Version)
	}
	if strings.TrimSpace(options.ObfsOpts.Mode) != "" {
		return nil, fmt.Errorf("snell inbound obfs is not supported")
	}
...
func (s *Snell) Listen(tunnel C.Tunnel) error {
	var err error
	s.l, err = snellListener.New(s.snell, tunnel, s.Additions()...)
	if err != nil {
		return err
	}
