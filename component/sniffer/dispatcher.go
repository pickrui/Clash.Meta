...
func (sd *Dispatcher) UDPSniff(packet C.PacketAdapter, packetSender C.PacketSender) C.PacketSender {
	...
	if sd.shouldOverride(metadata) {
		// A multi-packet wrapper must not hide a complete datagram protocol such
		// as STUN when both sniffers cover the same destination port.
		var deferred sniffer.MultiPacketSniffer
		var deferredReplace sniffer.ReplaceDomain
			...
			if current.supportsNetwork(C.UDP) {
				...
				if inWhitelist {
					...
					if wrapable, ok := current.Sniffer.(sniffer.MultiPacketSniffer); ok {
						if deferred == nil {
							deferred, deferredReplace = wrapable, replaceDomain
						}
						continue
					}
		...
		if deferred != nil {
			return deferred.WrapperSender(packetSender, deferredReplace)
		}
	}
