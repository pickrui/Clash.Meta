...
const (
	...
	// Bound per-packet decryption scratch independently of retained CRYPTO data.
	maxInitialPacketSize = 64 * (1 << 10)
)
...
func (q *quicPacketSender) readQUICPacket(b []byte, coalesced bool) (int, error) {
	...
	if packetEnd > maxInitialPacketSize {
		return 0, io.ErrShortBuffer
	}
	// Scratch stores the header-protection mask, copied header and ciphertext.
	// A full-size datagram can exceed NewPacket's fixed buffer even when its
	// wire length fits; reserve the mask in addition to the validated packet.
	cache := buf.NewSize(packetEnd + aes.BlockSize)
	defer cache.Release()
